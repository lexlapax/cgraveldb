...
func TestIndexInvertedIndex(t *testing.T) {
	...
	//indices
	indices,_ := idxc.Indices()
	assert.Equal(t, 1, len(indices))
	assert.Equal(t, idx.Key(), indices[0])
	idx2, err := idxc.AddIndex("test2")
	indices,_ = idxc.Indices()
	assert.Equal(t, 2, len(indices))
	stringset.Clear()
	stringset.AddArray(indices)
	assert.Equal(t, 2, stringset.Count())
	assert.True(t, stringset.Contains(idx.Key()))
	assert.True(t, stringset.Contains(idx2.Key()))
	assert.Equal(t, uint(0), idx2.DocCount())
	assert.Equal(t, uint(0), idx2.TokenCount())

	idx3, err := idxc.AddIndex("test3")
	indices,_ = idxc.Indices()
	...
	assert.True(t, stringset.Contains(idx.Key()))
	assert.True(t, stringset.Contains(idx2.Key()))
	assert.True(t, stringset.Contains(idx3.Key()))
	assert.Equal(t, uint(0), idx3.DocCount())
	assert.Equal(t, uint(0), idx3.TokenCount())
